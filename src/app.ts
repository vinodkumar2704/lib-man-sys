...
// userReg=>login=>show/add/borrow/return/exit
...
function userLogin() {
  ...
  while(!inputId || inputId === ''){
    inputId = prompt('Enter user id: ') as string;
  }
...
function userAction(user: UserInterface) {
  let option = prompt(
    "Welcome " +
      user.name +
  ...
  switch (option) {
    ...
    case "2":
      ...
      while (!isbn || books.some((b) => b.ISBN === isbn)) {
        isbn = parseInt(prompt("Enter book isbn:") as string);
        //check unique isbn
      }
