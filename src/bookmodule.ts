...
function addNewBook(title: string, author: string, isbn: number) {
...
function borrowBook(user: UserInterface,title:string) {
...
function returnBook(user: UserInterface, title: string) {
  const books = memoryInstance.booksCollection;
  if (!books.some((b) => b.title === title)) {
